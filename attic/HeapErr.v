...
Definition HeapSpec := Def ADT {
  ...
  (* Allocation returns the address for the newly allocated block.
     ...
     no such limits. *)
  Def Method1 allocS (r : rep) (len : Size | 0 < len) : rep * (Error + Ptr) :=
    addr <- { addr : Error + N
            | forall addr', addr = inr addr' ->
                P.for_all (fun a sz => negb (overlaps_bool a sz addr' (` len)))
                          (resvs r) };
    ret (match addr with
         | inl _    => (r, addr)
         | inr addr =>
           ({| resvs := M.add addr (` len) (resvs r)
             ; bytes := bytes r |}, inr addr)
         end),

  (* Freeing an unallocated block is a no-op. *)
  Def Method1 freeS (r : rep) (addr : Ptr) : rep * (Error + unit) :=
    eu <- { eu : Error + unit | True };
    ret (match eu with
         | inl _ => (r, eu)
         | inr _ => ({| resvs := M.remove addr (resvs r)
                      ; bytes := bytes r |}, inr tt)
         end),
  ...
  Def Method2 reallocS (r : rep) (addr : Ptr) (len : Size | 0 < len) :
    rep * (Error + Ptr) :=
    naddr <- { naddr : Error + N
             | forall addr', naddr = inr addr' ->
                 P.for_all (fun a sz => negb (overlaps_bool a sz addr' (` len)))
                           (M.remove addr (resvs r)) };
    ret (match naddr with
         | inl _    => (r, naddr)
         | inr naddr =>
           ({| resvs := M.add naddr (` len) (M.remove addr (resvs r))
             ; bytes :=
                 Ifopt M.find addr (resvs r) as sz
                 Then copy_bytes addr naddr (N.min sz (` len)) (bytes r)
                 Else bytes r |}, inr naddr)
         end),

  (* Peeking an uninitialized address allows any value to be returned. *)
  Def Method1 peekS (r : rep) (addr : Ptr) : rep * (Error + Word) :=
    p <- { p : Error + Word
         | forall v, M.MapsTo addr v (bytes r) -> p = inr v };
    ret (r, p),

  Def Method2 pokeS (r : rep) (addr : Ptr) (w : Word) : rep * (Error + unit) :=
    eu <- { eu : Error + unit | True };
    ret (match eu with
         | inl _ => (r, eu)
         | inr _ =>
           ({| resvs := resvs r
             ; bytes := M.add addr w (bytes r) |}, inr tt)
         end),
  ...
  Def Method3 memcpyS (r : rep) (addr1 : Ptr) (addr2 : Ptr) (len : Size) :
    ...
    ret (match eu with
         ...
         | inr _ =>
           ({| resvs := resvs r
             ; bytes := copy_bytes addr1 addr2 len (bytes r) |}, inr tt)
         end),
